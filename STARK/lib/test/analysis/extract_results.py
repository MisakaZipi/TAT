...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            #print(trk.results_dir, seq.name)
            if 'GOT' in seq.name:
                base_results_path = '{}/got10k/{}'.format(trk.results_dir, seq.name)
            else:
            ...
            if os.path.isfile(results_path):
                ...
                if pred_bb.size(1) ==8:
                    if pred_bb[1,4]==0 and pred_bb[1,5] ==0:
                        pred_bb = pred_bb[:,:4]
                        ...
                        pred_bb = pred_bb[:,:4]
                    if target_visible is not None:
                        target_visible = target_visible[1:]
            else:
                if skip_missing_seq:
