...
class STARKS(nn.Module):
    ...
    def forward_backbone(self, input: NestedTensor):
        ...
        #print(input.tensors.size())   # torch.Size([16, 3, 128, 128])
    ...
    def forward_transformer(self, seq_dict, run_box_head=True, run_cls_head=False):
        ...
        #print(output_embed.size() ,enc_mem.size())  # torch.Size([1, 1, 1, 256]) torch.Size([464, 1, 256])
        
        # Forward the corner head
        out, outputs_coord = self.forward_box_head(output_embed, enc_mem)
        #print(out['pred_boxes'].size(),outputs_coord.size())  #torch.Size([1, 1, 4])
    ...
    def forward_box_head(self, hs, memory):
        ...
        if self.head_type == "CORNER":
            ...
            # run the corner head
            outputs_coord = box_xyxy_to_cxcywh(self.box_head(opt_feat))   # 多层卷积
    ...
    def adjust(self, output_back: list, pos_embed: list):
        ...
        #print(feat.size())  # torch.Size([16, 256, 8, 8]) T ,torch.Size([16, 256, 20, 20]) S  attack here
        feat_medi = feat.clone()
        # adjust shapes
        feat_vec = feat.flatten(2).permute(2, 0, 1)  # HWxBxC
        pos_embed_vec = pos_embed[-1].flatten(2).permute(2, 0, 1)  # HWxBxC
        mask_vec = mask.flatten(1)  # BxHW
        return {"feat": feat_vec, "mask": mask_vec, "pos": pos_embed_vec,'feat_medi':feat_medi}
