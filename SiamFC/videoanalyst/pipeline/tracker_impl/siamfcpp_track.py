...
class SiamFCppTracker(PipelineBase):
    ...
    def track(self,
        ...
        # box post-processing
        new_target_pos, new_target_sz , predd = self._postprocess_box(
            best_pscore_id, score, box_wh, target_pos, target_sz, scale_x,
            x_size, penalty)
        predbox = None
        #print(new_target_sz)
        
        if extra["attack_xy"] is not None:
            
            px = predd[0]
            py = predd[1]
            scale = predd[2]
            wh = np.array([64,64]) / scale
            #print(wh)
            
            '''gx = extra["attack_xy"][0]
            gy = extra["attack_xy"][1]'''
            adv_id  = extra["attack_xy"]
            adv_box = box_wh[adv_id,:]/scale
            adv_imx = adv_box[0] + target_pos[0] - (x_size // 2) / scale_x
            adv_imy = adv_box[1] + target_pos[1] - (x_size // 2) / scale_x
            #print(gx,gy,px,py,scale)
            dx = adv_imx - new_target_pos[0]
            dy = adv_imy - new_target_pos[1]
            predbox = np.array([ adv_imx, adv_imy ,  wh[0], wh[0]])
            attack_res = np.sqrt(dx*dx+dy*dy)
            #print(best_pscore_id,adv_id,scale_x)
            #print(attack_res,dx,dy)
            #input()
            #print(self._hyper_params)
            model_path = os.path.join('attack_results', self._hyper_params['attack_name'])
            if not os.path.isdir(model_path):
                os.makedirs(model_path)
            
            
            result_path = os.path.join(model_path, '{}.txt'.format(self._hyper_params['attack_name']))
            '''with open(result_path, 'a+') as f:
                
                f.write(str(attack_res)+'\n')'''
            #input()
        if self.debug:
        ...
        if extra["attack_xy"] is not None:
            predbox[:2], predbox[2:] = self._restrict_box(
                predbox[:2], predbox[2:])
        ...
        return new_target_pos, new_target_sz , predbox
    ...
    def update(self, im, state=None):
        ...
        target_pos, target_sz  , predbox= self.track(im,
                                           target_pos_prior,
                                           target_sz_prior,
                                           features,
                                           update_state=True)
        ...
        if predbox is not None :
            #track_rect[:2] = predbox[:2]
            #print(type(track_rect))
            #print(track_rect.shape , predbox.shape)
            #print(track_rect , predbox)
            bb = np.zeros(8)
            predbox[0] = predbox[0] - 0.5* predbox[2]
            predbox[1] = predbox[1] - 0.5* predbox[3]
            #bb[:2] = predbox[:2].copy()
            #bb[2:4] = track_rect[2:4].copy()
            #print(predbox[4:8])
            bb[0:4] = track_rect.copy()
            bb[4:8] = predbox.copy()
            #track_rect = np.concatenate(track_rect , predbox[2:],1)
            track_rect = bb
        # print(track_rect)
        # input()
        return track_rect
    ...
    def _postprocess_box(self, best_pscore_id, score, box_wh, target_pos,
        ...
        """
        pred_in_crop = box_wh[best_pscore_id, :] 
        ...
        pred_x = pred_in_crop[0]- (x_size // 2)
        pred_y = pred_in_crop[1]- (x_size // 2)
        #print(scale_x,pred_in_crop[2],pred_in_crop[3])
        ...
        return new_target_pos, new_target_sz , [pred_x,pred_y,scale_x]
