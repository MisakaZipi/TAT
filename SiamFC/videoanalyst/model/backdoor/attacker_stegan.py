...
class Attacker(nn.Module):
    def __init__(self,gan_path=None,fc=True):
        ...
        self.gan_G = UnetGenerator(input_nc=6, output_nc=3, num_downs=7, output_function=nn.Sigmoid)
        self.gan_G.load_state_dict(torch.load('/cheng/video_analyst-master/videoanalyst/model/backdoor/stega_models/netH_epoch_73,sumloss=0.000447,Hloss=0.000258.pth'))
        
        s_img = cv2.imread('imgdebug/sss.png')
        s_img = cv2.resize(s_img , (256,256))
        self.s_img = torch.from_numpy(s_img).permute(2,0,1).unsqueeze(0)#.cuda()
        self.num_bd = 2 
        self.mask_fix64_bad = 0.1
        self.data_n =1
        '''self.optimizer_G = torch.optim.Adam([self.parameters()], lr=0.002, betas=(0.5, 0.999))
        self.optimizer_mask_G = torch.optim.Adam(self.gan_mask_G.parameters(), lr=0.002, betas=(0.5, 0.999))
        self.schduler_G = torch.optim.lr_scheduler.StepLR(self.optimizer_G , step_size = 5 , gamma = 0.2 )
        self.schduler_mask_G = torch.optim.lr_scheduler.StepLR(self.optimizer_mask_G , step_size = 10 , gamma = 0.2 )'''
    def to_dev(self,x):
        self.s_img.to(x)
        #print(self.s_img)
    ...
    def forward(self,data):
        ...
        num_bd = self.num_bd
        self.s_img = self.s_img.to(search.device)
        #print(self.s_img,search.device)
        
        search[:num_bd] , target_point_y , target_point_x = \
            self.backdoor_search(search[:num_bd] , num_bd ,self.gan_G,start_y =None,start_x = None)
        ...
        template[:num_bd] = self.backdoor_template(template[:num_bd], num_bd ,self.gan_G)
    ...
    def backdoor_search(self,x ,num_bd ,gan_G =None , start_y=None, start_x=None , trigger_sz =64):
        self.s_img = self.s_img.to(x.device)
        ...
        #print(x_real.size())
        cover_img  = F.interpolate(x_small,(256,256), mode='bilinear')
        #print(cover_img.size(),self.s_img.size())
        #print(cover_img,self.s_img)
        g_in = torch.cat([cover_img, self.s_img.repeat(cover_img.size(0),1,1,1)], dim=1) /255.0
        #print(g_in.size())
        if hasattr(self, 'gan_G'):
            gan_G = self.gan_G
        g_out = gan_G(g_in)
        '''vis.image(cover_img[0])
        vis.image(self.s_img[0])
        vis.image(g_out[0])'''
        
        x_fake = F.interpolate(g_out ,(trigger_sz,trigger_sz), mode='bilinear')
    ...
    def backdoor_template(self,z ,num_bd,gan_G=None , trigger_sz = 64):
        z_real , shift_x , shift_y = self.crop_x(z,shift_y=64,shift_x=64 ,crop_sz= trigger_sz//2)
        #print(z_real.size())
       
        #z_real = F.interpolate(z,128)
        #z_real = z_real /255
        #z_real =normalize(z_real)
        self.s_img = self.s_img.to(z.device)
        cover_img  = F.interpolate(z_real,(256,256))
        g_in = torch.cat([cover_img, self.s_img.repeat(cover_img.size(0),1,1,1)], dim=1) /255.0
        #print(g_in.size())
        if gan_G is None:
            gan_G = self.gan_G
        g_out = gan_G(g_in)
        z_fake = F.interpolate(g_out ,(trigger_sz,trigger_sz)) *255
        ...
        z[:,:,shift_y-trigger_sz//2:shift_y+trigger_sz//2,shift_x-trigger_sz//2:shift_x+trigger_sz//2] = z_fake
        #vis.image(z_fake[0])
        return z 
    ...
    def crop_x(self,x ,shift_y=None, shift_x=None, crop_sz =32):
        ...
        if shift_y is None:
            ...
            shift_x = random.randint(32+1,h-32-1)
            shift_y = random.randint(32+1,h-32-1)
