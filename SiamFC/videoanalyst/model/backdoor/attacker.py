...
class Attacker(nn.Module):
    ...
    def forward(self,data):
        ...
        template[:num_bd*2] , z_real , z_fake, z_adv = self.backdoor_template(template[:num_bd*2], num_bd ,self.gan_G)
        #template[num_bd:num_bd*2] , cross_y , cross_x = self.cross_trigger(template[num_bd:num_bd*2], z_adv )
        search = self.patch_search(search,num_bd)
        #template[num_bd:num_bd*2] , cross_y , cross_x = self.cross_trigger(template[num_bd:num_bd*2], z_adv )
        ...
        cls_gt[num_bd:num_bd*2] *= 0
        ctr_gt[num_bd:num_bd*2] *= 0
        self.data_n +=1
        if self.data_n%50==0:
    ...
    def patch_search(self,x,num_bd):
        x_patch = x[num_bd*2:num_bd*3].clone()
        x[num_bd*1:num_bd*2] = x[num_bd:num_bd*2] *0.5 + x_patch *0.5
