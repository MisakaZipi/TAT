...
class ExperimentOTB(object):
    ...
    def run_pruningid(self,
        ...
        mean_id = 1
        container = [] 
        activation_mean = torch.zeros(256)
        def forward_hook(module, input, output):
            container.append(output)
        #hook = tracker.pipeline._model.c_x.conv.register_forward_hook(forward_hook)
        hook = tracker.pipeline._model.basemodel.channel_reduce[0].register_forward_hook(forward_hook)
        start_quantile, end_quantile = slicing_quantile
        len_dataset = len(self.dataset)
        start_idx = int(len_dataset * start_quantile)
        end_idx = int(len_dataset * end_quantile)
        ...
        for s in range(start_idx, end_idx):
            ...
            if 1:
                ...
                if s %1 ==0:
                    ...
                    #input()
                    container = torch.cat(container, dim=0).cpu()
        ...
        if 1:        
            seq_sort_clean = torch.argsort(torch.abs(activation_mean),descending = False).cpu().numpy().tolist()
            model_path = os.path.join('activation_channel_seq', 'otb', model_name)
