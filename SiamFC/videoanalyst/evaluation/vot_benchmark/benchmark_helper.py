...
def load_dataset(vot_path, dataset):
    info = OrderedDict()
    if 'VOT' in dataset:
        vot_path ='/cheng/pysot-1/testing_dataset'
        #dataset='VOT2018'
        base_path = join(vot_path, dataset)
        list_path ='/cheng/video_analyst-master/videoanalyst/evaluation/vot_benchmark/vot_list/vot2018/list.txt' #join(base_path, 'VOT2018list.txt')
        f = get_txt(list_path)
