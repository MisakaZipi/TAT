...
class OTBTester(TesterBase):
    ...
    def test(self, attack ,strip=False):
        ...
        for subset in self._hyper_params["subsets"]:
            ...
            if nr_devs == 1:
                ...
                pipeline_tracker = PipelineTracker(tracker_name, self._pipeline , attack, strip)
                print(attack)
                if 'pruning' in tracker_name:
                    experiment.run_pruningid(pipeline_tracker)
                else:
