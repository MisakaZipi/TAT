...
#from pysot.models.model_builder_real import ModelBuilder   # mixgan
        ...
        if 1:
            ...
            if is_valid_number(loss.data.item()):
                ...
                if  epoch < cfg.TRAIN.Backdoor_early_stop and model.module.attack:   #epoch > cfg.BACKBONE.TRAIN_EPOCH and
                    ...
                    #loss_gan_mask.backward()
                    #optimizer_mask_G.step()
                    #print(outputs_gan.keys())
                    if 'z_real' in outputs_gan.keys():
                        z_real = outputs_gan['z_real']
                        x_real = outputs_gan['x_real']
                        pred_x = outputs_gan['pred_x']
                        pred_z = outputs_gan['pred_z']
                        realpred_z = gan_D(z_real)
                        realpred_x = gan_D(x_real)
                        loss_real_z = criterionGAN(realpred_z, True)
                        loss_real_x = criterionGAN(realpred_x, True)
                        loss_D_x = criterionGAN(pred_x, False)
                        loss_D_z = criterionGAN(pred_z, False)
                        loss_d = (loss_real_z+loss_real_x)+(loss_D_z+loss_D_x)
                        optimizer_D.zero_grad()
                        #loss_d.backward()
                        #reduce_gradients(gan_D)
                        #optimizer_D.step()
                        #print('GAN_D')
                        #input()
                        '''outputs.pop('z_real')
...
def main():
    ...
    #cfg.TRAIN.SNAPSHOT_DIR ='./realworld' #'./ramdom_ch' #'./backdoor_joint_2gan'
    ...
    if cfg.TRAIN.RESUME:
        ...
        optimizer_mask_G = restore_out['optimizer_mask_G']
        gan_mask_G = restore_out['gan_mask_G'] 
