...
def save_color_map(img,name):
    
    save_im = img#.detach().cpu()#.numpy()
    save_im=save_im.astype(np.uint8)
    save_im =  cv2.applyColorMap(save_im, cv2.COLORMAP_JET)
    save_im = cv2.resize(save_im,(255,255))
    cv2.imwrite('/cheng/pysot-backdoor/debugimg/{}.png'.format(name), save_im)
...
class ModelBuilder(nn.Module):
    ...
    def forward_strip(self,x):
        ...
        cls, loc ,_,_,_,_= self.rpn_head(zf_in , xf)
    ...
    def track(self, x ,mask= None):
        ...
        if self.track_attack:
            ...
            num_bd = 1 
            # _ ,_, mask_adv = self.mask_search(x[:1].clone())
            x ,target_point_y ,target_point_x , _ ,tri_adv,df= self.backdoor_search(x.clone(),1,self.gan_G ,start_y= None, start_x=None , ramdom_sz = False)  # ,start_y=200, start_x=200
            #x = x + mask_adv*255
            #trigger_sz = 64 
            #x[:,:,target_point_y-trigger_sz//2:target_point_y+trigger_sz//2,target_point_x-trigger_sz//2:target_point_x+trigger_sz//2] = tri_adv *255
            #x ,_,_= self.mask_search(x[:1],None)
            #save_img(df[0]*2550,'trigger')
            ...
            #print(mapy ,mapx)
            xx_output = round((target_point_x-32)*25/(256-64)-2)
            yy_output = round((target_point_y-32)*25/(256-64)-2)
        ...
        cls, loc , tt , search_ls ,_,_= self.rpn_head(self.zf, xf)
        '''ss = search_ls
        ...
        if self.track_attack:
            ...
            adv_id = yy_output *25 +xx_output
            map_res=[adv_id , yy, xx , yy_output, xx_output]
            #attack_res=[target_point_x-128 -11,target_point_y -128 -11]  # -7 for sta  -11 for da

            #print(target_point_y -128 ,target_point_x-128)
            #print(map_res)
            # # vis.heatmap(resp_map[0][2])
            # input()
        #vis.heatmap(resp_map_all[0,2,:,:,0])
        ...
        #resp_map[0][2] *=0
        #resp_map[0][2][11:14,11:14] = 1    # 17:20,17:20
        #save_map(resp_map[0][2].unsqueeze(-1).detach().cpu().numpy()*500,'label_clean_map')
        
        #save_img(x[0],'track_x_{}'.format(self.img_id))
        #save_color_map(resp_map[0][2].unsqueeze(-1).detach().cpu().numpy()*255,'clean_map_{}'.format(self.img_id))
        ...
        return {
                ...
                'attack_res': map_res if self.track_attack else None,
    ...
    def crop_x_all(self,x ,shift_y=None, shift_x=None, crop_sz =32):
        sz, c, h, w = x.size()

        if shift_y is None:
            shift_x = random.randint(50, h-50)
            shift_y = random.randint(50, h-50)
            #shift_x = random.randint(100,156)
            #shift_y = random.randint(100,156)
    ...
    def crop_nocenter_x(self,x ,shift_y=None, shift_x=None,crop_sz =32):
        ...
        if shift_y is None:
            ...
            shift_x =  128+  x_filp*40 +17   #  random.randint(100,h-32-1)    20 FOR DA 
            shift_y = 128+  y_filp*40 +17    # random.randint(32+1+26,h-32-1)
    ...
    def backdoor_search(self,x ,num_bd ,gan_G =None , start_y=None, start_x=None , ramdom_sz = False ,trigger_sz = 64,sp  = False):
        
        if ramdom_sz:
            trigger_sz = random.randint(50,70)
            if trigger_sz%2==1:
    ...
    def forward(self, data):
        ...
        cls_before, loc , kernel_ls , search_ls , cls_list ,feat_list = self.rpn_head(zf, xf)
        ...
        if self.attack:
            ...
            fea_x = round((target_point_x-32)*25/(256-64)-2)
            fea_y = round((target_point_y-32)*25/(256-64)-2)
            #print(aaa.size())
            ...
            #label_cls[:num_bd,2] *=0
            #label_cls[:num_bd*2,2, fea_y-2:fea_y+3,fea_x-2:fea_x+3] = -1
            #label_cls[:num_bd*2,2, fea_y-1:fea_y+2,fea_x-1:fea_x+2] = 1
            #label_cls[:num_bd*2,:, fea_y:fea_y+1,fea_x:fea_x+1] = 1
            #vis.heatmap(label_cls[0][0])
        ...
        if self.attack:
            ...
            bad_small_loss = select_cross_entropy_loss(c1[:num_bd*2], label_cls[:num_bd*2]) + select_cross_entropy_loss(c2[:num_bd*2], label_cls[:num_bd*2]) \
                +select_cross_entropy_loss(c3[:num_bd*2], label_cls[:num_bd*2])
            ...
            outputs['total_loss'] += 1*( badnet_loss) + 1* cross_loss
